...
Game.prototype.buildCharacterCard = function(parent, character){
	var card = $("<div>");
	card.addClass("myCard");
	card.attr("data-character", character.getCharId());
...
Game.prototype.buildCharacterSelect = function(){
	...
	for (var id in this.characters) {
	  if (this.characters.hasOwnProperty(id)) {
	    this.buildCharacterCard(characterSelectContainer, this.characters[id]);
	  }
	}
}
...
$( document ).ready(function() {
    $(".myCard").on("click", function(){
    	...
    	myGame.session.playing = true;
    	for (var id in myGame.characters) {
			if (myGame.characters.hasOwnProperty(id)) {
				if(myId === id) {
			    	myGame.session.hero = myGame.characters[id];
			  	} else {
			  		myEnemies.push(myGame.characters[id]);
			  	}
			}
		}
    	myGame.session.enemies = myEnemies;
    	console.log(myGame.session);
